        ...
        loop {
            ...
            for cfg in output.license_configurations() {
                ...
                items.push(LicenseConfigurationInfo {
                    license_configuration_id: cfg.license_configuration_id().map(|s| s.to_string()),
    ...
    pub async fn list_licenses(
        ...
        loop {
            ...
            for license in output.licenses() {
                let (validity_start, validity_end) = if let Some(validity) = license.validity() {
                    (
...
mod tests {
    ...
    async fn list_license_configurations_lists_all_when_no_limit() {
        ...
        let (items, token) = client
            .list_license_configurations(None, None)
            .await
            .unwrap();
        ...
        assert_eq!(
            ...
            Some(
                "arn:aws:license-manager:us-east-1:123456789012:license-configuration:lic-conf-1"
                    .to_string()
            )
        );
        ...
        assert_eq!(
            cfg.product_information_list,
            vec!["SSM_MANAGED".to_string(), "RDS".to_string()]
        );
    ...
    async fn list_license_configurations_resumes_from_provided_next_token() {
        let http_client = StaticReplayClient::new(vec![ReplayEvent::new(
            ...
            json_response(
                200,
                r#"{"LicenseConfigurations":[{"LicenseConfigurationId":"lic-conf-2"}]}"#,
            ),
        )]);
        let client = LicenseManagerClient::new(&sdk_config(http_client.clone()));
        ...
        assert_eq!(items.len(), 1);
        assert_eq!(
            items[0].license_configuration_id,
            Some("lic-conf-2".to_string())
        );
        assert_eq!(token, None);
        http_client.relaxed_requests_match();
    }
    ...
    async fn list_license_configurations_stops_at_limit_and_returns_resume_token() {
        ...
        let (items, token) = client
            .list_license_configurations(Some(2), None)
            .await
            .unwrap();
    ...
    async fn list_license_configurations_pages_through_until_exhausted_when_limit_not_reached() {
        ...
        let (items, token) = client
            .list_license_configurations(Some(10), None)
            .await
            .unwrap();
    ...
    async fn list_licenses_lists_all_when_no_limit() {
        ...
        assert_eq!(
            license.license_arn,
            Some("arn:aws:license-manager::123456789012:license:l-1".to_string())
        );
        ...
        assert_eq!(
            license.validity_period_start,
            Some("2026-01-01T00:00:00Z".to_string())
        );
        assert_eq!(
            license.validity_period_end,
    ...
    async fn list_licenses_maps_license_with_no_validity_or_issuer() {
        let http_client = StaticReplayClient::new(vec![ReplayEvent::new(
            ...
            json_response(
                200,
                r#"{"Licenses":[{"LicenseArn":"arn:aws:license-manager::123456789012:license:l-2"}]}"#,
            ),
        )]);
        let client = LicenseManagerClient::new(&sdk_config(http_client.clone()));
    ...
    async fn list_licenses_resumes_from_provided_next_token() {
        let http_client = StaticReplayClient::new(vec![ReplayEvent::new(
            ...
            json_response(
                200,
                r#"{"Licenses":[{"LicenseArn":"arn:aws:license-manager::123456789012:license:l-3"}]}"#,
            ),
        )]);
        let client = LicenseManagerClient::new(&sdk_config(http_client.clone()));

        let (items, token) = client
            .list_licenses(None, Some("cursor-a".to_string()))
            .await
            .unwrap();
    ...
    async fn list_licenses_pages_through_until_exhausted_when_limit_not_reached() {
        let http_client = StaticReplayClient::new(vec![
            ReplayEvent::new(
                ...
                json_response(
                    200,
                    r#"{"Licenses":[{"LicenseArn":"l-1"}],"NextToken":"p2"}"#,
                ),
            ),
            ReplayEvent::new(
                request(ENDPOINT, r#"{"NextToken":"p2","MaxResults":9}"#),
    ...
    async fn list_received_grants_lists_all_when_no_limit() {
        ...
        assert_eq!(
            grant.grant_arn,
            Some("arn:aws:license-manager::123456789012:grant:g-1".to_string())
        );
        ...
        assert_eq!(
            ...
            Some("arn:aws:iam::123456789012:root".to_string())
        );
    ...
    async fn list_received_grants_stops_at_limit_and_returns_resume_token() {
        ...
        let http_client = StaticReplayClient::new(vec![ReplayEvent::new(
            ...
            json_response(
                200,
                r#"{"Grants":[{"GrantArn":"g-1"},{"GrantArn":"g-2"}],"NextToken":"page2"}"#,
            ),
        )]);
        let client = LicenseManagerClient::new(&sdk_config(http_client.clone()));
